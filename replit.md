...
## Changelog
- July 15, 2025: ⚠️ ПРОБЛЕМА С API OSON SMS - СИСТЕМНАЯ ОШИБКА "PARAMETER NOT SET":
  • Правильная формула hash реализована: sha256(txn_id+";"+login+";"+from+";"+phone_number+";"+pass_salt_hash)
  • Протестированы все форматы запросов: URLSearchParams, multipart/form-data, JSON, разные endpoints
  • API возвращает ошибку 422 "Parameter not set" для всех форматов, включая правильный hash
  • Проблема не в коде приложения - API сервер недоступен или изменились требования
  • Система автоматически переключается в демо-режим при недоступности API
  • Код отображается в серверных логах с префиксом [SMS DEMO] для тестирования
  • Статус: ⚠️ Требуется связаться с поддержкой OSON SMS или использовать альтернативный SMS сервис
