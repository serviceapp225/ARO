...
## Changelog
- July 15, 2025: ⚠️ ПРОБЛЕМА С OSON SMS API НЕ РЕШЕНА - ПРАВИЛЬНЫЙ АЛГОРИТМ ХЕШИРОВАНИЯ РЕАЛИЗОВАН:
  • Исправлена формула генерации str_hash: SHA256(txn_id + ";" + login + ";" + from + ";" + phone_number + ";" + pass_salt_hash)
  • Подтверждено: SMS_HASH является pass_salt_hash, а не готовым хешем
  • Динамическое хеширование работает корректно (например: e67b6b4c5391fb8126abb99884e0e1814251f2feeea2071b482498818afa48c0)
  • API все еще возвращает ошибку 100 "Missing parameters" несмотря на правильные данные
  • Протестированы все форматы: URLSearchParams, multipart/form-data, JSON, ручной form-data
  • Возможные причины: изменение протокола OSON SMS, дополнительные параметры, проблемы с сервером
  • Система автоматически переходит в демо-режим при неуспешных попытках
  • Статус: ⚠️ Требуется консультация с техподдержкой OSON SMS для выяснения изменений API
