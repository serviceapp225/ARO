...
## Changelog
- July 16, 2025: ⚠️ SMS API ОБНОВЛЕН НО ТРЕБУЕТСЯ НАСТРОЙКА БЕЛОГО СПИСКА IP:
  • Система полностью переведена на новый GET API формат OSON SMS
  • Изменен метод запроса с POST на GET с параметрами в URL
  • Обновлен формат номера телефона: +992903331332 → 903331332 (9 цифр)
  • Формула хеширования осталась той же: SHA256(txn_id + ";" + login + ";" + from + ";" + phone_number + ";" + pass_salt_hash)
  • Обновлены обе функции: sendSMSCode и sendSMSNotification
  • Пример нового формата: GET /sendsms.php?from=OsonSMS&phone_number=903331332&msg=Kod%3A%20123456&str_hash=abc123...&txn_id=autobid_123&login=zarex
  • Обновлена документация в SMS_INTEGRATION_GUIDE.md
  • ПРОБЛЕМА: Replit использует динамические IP адреса (34.100.160.130 → 34.93.213.223 при перезапуске)
  • Система автоматически переходит в демо-режим при недоступности API
  • Статус: ⚠️ SMS интеграция готова технически, но требуется добавление всех IP Replit в белый список OSON SMS
