...
## Changelog
- July 15, 2025: ⚠️ ПОДТВЕРЖДЕНА ПРОБЛЕМА С API OSON SMS - СИСТЕМНАЯ НЕДОСТУПНОСТЬ:
  • ПОДТВЕРЖДЕНИЕ: Прямой curl запрос к api.osonsms.com возвращает ошибку "Parameter not set" даже с правильными параметрами
  • ДИАГНОСТИКА: Все форматы запросов (URLSearchParams, multipart/form-data, JSON) возвращают ошибку 100 "fatal"
  • СТАРЫЙ ENDPOINT: sendsms.php полностью недоступен (404 Not Found)
  • НОВЫЙ ENDPOINT: sendsms_v1.php доступен но не принимает параметры
  • СИСТЕМНАЯ ПРОБЛЕМА: API изменился после 23 июня 2025 года
  • АВТОМАТИЧЕСКОЕ ПЕРЕКЛЮЧЕНИЕ: Система стабильно работает в демо-режиме
  • КОДЫ В ЛОГАХ: Все SMS коды отображаются в серверных логах с префиксом [SMS DEMO]
  • СТАТУС: ✅ Приложение полностью функционально в демо-режиме, требуется связаться с OSON SMS для восстановления реального SMS
