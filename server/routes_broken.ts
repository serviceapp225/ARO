...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/listings/:id/photo/:index", async (req, res) => {
    try {
      ...
      // Get photos from storage with error handling
      let listing;
      try {
        listing = await storage.getListing(listingId);
      } catch (storageError) {
        console.error("Error fetching photo:", storageError);
      ...
      if (requestedPhotoIndex >= 0 && requestedPhotoIndex < photoArray.length) {
        // Generate a simple placeholder SVG for the car listing
        ...
        // Return 404 for invalid photo index
