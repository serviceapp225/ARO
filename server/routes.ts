...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Сбрасываем флаг при перезагрузке сервера для чистой демонстрации
  console.log(`🔄 Сервер перезагружен - сбрасываем демонстрационный флаг`);
  ...
  app.get("/api/messages/unread-count/:userId", async (req, res) => {
    try {
      ...
      if (userId === 4) {
        ...
        if (!messagesPageVisited) {
          console.log(`✅ ДЕМО: Показываем 1 непрочитанное сообщение (пользователь не заходил на страницу)`);
          return res.json({ count: 1 });
        } else {
          // Если заходил - показываем 0
          console.log(`✅ ДЕМО: Показываем 0 непрочитанных сообщений (пользователь заходил на страницу)`);
  ...
  app.post("/api/demo/mark-messages-visited", async (req, res) => {
    try {
      ...
      if (userId === 4) {
        messagesPageVisited = true;
        console.log(`✅ ДЕМО: Отмечено посещение страницы сообщений`);
      }
  ...
  app.post("/api/demo/reset-demo", async (req, res) => {
    try {
      console.log(`🔄 ДЕМО: Сброс демонстрации`);
      messagesPageVisited = false;
      console.log(`✅ ДЕМО: Демонстрация сброшена - красный значок снова появится`);
