...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/messages/unread-count/:userId", async (req, res) => {
    try {
      ...
      // ВРЕМЕННО: показываем красный значок для демонстрации
      // Для пользователя 4 (админ) показываем 2 непрочитанных сообщения
      if (userId === 4) {
        console.log(`✅ ДЕМО: Показываем 2 непрочитанных сообщения для пользователя ${userId}`);
