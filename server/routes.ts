...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ВРЕМЕННО ОТКЛЮЧЕНО для стабилизации деплоя - слишком частые обновления кэша
  // setInterval(updateListingsCache, 2000);
  
  // ВРЕМЕННО ОТКЛЮЧЕНО для стабилизации деплоя
  // setTimeout(() => {
  ...
  app.post("/api/listings", upload.any(), (req, res, next) => {
    ...
    try {
      ...
      if (uploadedFiles && Array.isArray(uploadedFiles) && uploadedFiles.length > 0) {
        console.log(`📁 СТАРТ СОХРАНЕНИЯ: Начинаем сохранение ${uploadedFiles.length} фотографий для объявления ${listing.id}`);
        console.log(`🗂️ ПУТЬ: Файлы будут сохранены в uploads/listings/${Math.floor(listing.id / 1000) * 1000}/${listing.id}/`);
        
        for (let i = 0; i < uploadedFiles.length; i++) {
          console.log(`📸 ФОТО ${i + 1}/${uploadedFiles.length}: Начинаем обработку...`);
          const file = uploadedFiles[i];
          
          if (file && file.buffer) {
            console.log(`📸 ФОТО ${i + 1}: Обрабатываем файл, размер: ${(file.buffer.length / 1024).toFixed(1)}KB`);
        ...
        console.log(`📊 ИТОГИ СОХРАНЕНИЯ: Успешно обработано ${fileNames.length} из ${uploadedFiles.length} фотографий`);
  ...
  app.post("/api/listings/:id/bids", sanitizeInput, getUserFromContext, async (req, res) => {
    ...
    try {
      ...
      // МЯГКОЕ ОБНОВЛЕНИЕ: Получаем свежие данные без агрессивной очистки кэша
      // Не очищаем полностью кэш чтобы избежать временного исчезновения аукционов
      ...
      // УМНОЕ ОБНОВЛЕНИЕ КЭША: Обновляем только конкретный аукцион, не очищая все данные
      console.log('🎯 Умное обновление кэша для мгновенной отзывчивости без потери данных');
      
      // Очищаем только кэш конкретного аукциона, не все карточки
      clearCachePattern(`listing_${listingId}`);
      console.log(`🎯 Очищен кэш только для аукциона ${listingId} - избегаем потери данных`);
      
      // ПЛАВНОЕ ОБНОВЛЕНИЕ КЭША: Обновляем конкретный аукцион без потери остальных данных
      console.log(`🚀 Плавное обновление кэша для аукциона ${listingId}: current_bid ${validatedData.amount}`);
      
      // Находим и обновляем аукцион в кэше, сохраняя все остальные данные
      ...
      // Мягко обновляем время последнего обновления конкретного аукциона  
      // НЕ изменяем lastCacheUpdate чтобы не сбросить все кэши
      
      // Запускаем выборочное обновление только затронутого аукциона в фоне
      storage.getListing(listingId).then(freshListing => {
        if (freshListing && cacheIndex !== -1) {
          cachedListings[cacheIndex] = freshListing;
          console.log(`🔄 Фоновое обновление аукциона ${listingId} завершено`);
        }
      }).catch(error => {
