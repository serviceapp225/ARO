...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // NOCODB WEBHOOK ENDPOINTS
  
  // Webhook для одобрения объявления
  app.post("/api/webhooks/listing-approved", async (req, res) => {
    try {
      ...
      await storage.updateListingStatus(id, 'active');
      ...
      await storage.createNotification({
        ...
        message: `Ваше объявление ${listing.lotNumber} успешно одобрено и добавлено в аукцион`,
        listingId: id
      });
  ...
  // Webhook для отклонения объявления
  app.post("/api/webhooks/listing-rejected", async (req, res) => {
    try {
      ...
      // Обновляем статус объявления
      await storage.updateListingStatus(id, 'rejected');
      ...
      await storage.createNotification({
        ...
        message: `Ваше объявление ${listing.lotNumber} было отклонено. ${reason || 'Причина не указана'}`,
        listingId: id
      });
  ...
  // Webhook для активации пользователя
  app.post("/api/webhooks/user-activated", async (req, res) => {
    try {
      ...
      await storage.createNotification({
        ...
        message: 'Ваш аккаунт успешно активирован. Теперь вы можете полноценно пользоваться платформой'
      });
  ...
  // Webhook для деактивации пользователя
  app.post("/api/webhooks/user-deactivated", async (req, res) => {
    try {
      const { id, is_active, reason } = req.body;
      
      if (is_active) {
        return res.status(400).json({ error: "Invalid deactivation status" });
      }
      ...
      await storage.createNotification({
        ...
        message: `Ваш аккаунт был деактивирован. ${reason || 'Причина не указана'}`
      });
  ...
  // Webhook для создания массовых уведомлений
  app.post("/api/webhooks/broadcast-notification", async (req, res) => {
    try {
      ...
      // Если не указаны конкретные пользователи, отправляем всем активным
      if (!userIds || userIds.length === 0) {
        const allUsers = await storage.getAllUsers();
        userIds = allUsers.filter(u => u.isActive).map(u => u.id);
      }
  ...
  // API для получения статистики (для NocoDB дашборда)
  app.get("/api/admin/stats", async (req, res) => {
    try {
      const stats = await storage.getAdminStats();
      
      // Дополнительная статистика
      const totalBids = await storage.getBidCountForListing(0); // Общее количество ставок
      const allUsers = await storage.getAllUsers();
