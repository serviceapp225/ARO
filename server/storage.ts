...
import { eq, and, desc, sql, or, ilike, inArray, isNull, gte } from "drizzle-orm";
...
class PostgreSQLStorage implements IStorage {
  ...
  async getListingsByStatus(status: string, limit?: number): Promise<CarListing[]> {
    let query = db.select().from(carListings).where(eq(carListings.status, status)).orderBy(desc(carListings.createdAt));
    if (limit) {
  ...
  async updateListing(id: number, data: Partial<InsertCarListing>): Promise<CarListing | undefined> {
    const [listing] = await db
      .update(carListings)
      .set({ ...data, updatedAt: new Date() })
      .where(eq(carListings.id, id))
  ...
  async updateListingCurrentBid(id: number, amount: string): Promise<CarListing | undefined> {
    const [listing] = await db
      .update(carListings)
      .set({ currentBid: parseInt(amount), updatedAt: new Date() })
      .where(eq(carListings.id, id))
      .returning();
    return listing;
  }
  ...
  async searchListings(filters: {
    ...
    if (filters.minPrice) {
      conditions.push(sql`${carListings.startingPrice} >= ${filters.minPrice}`);
    }

    if (filters.maxPrice) {
      conditions.push(sql`${carListings.startingPrice} <= ${filters.maxPrice}`);
    }
  ...
  // Bid operations
  async getBidsForListing(listingId: number): Promise<Bid[]> {
    return await db.select().from(bids).where(eq(bids.listingId, listingId)).orderBy(desc(bids.amount));
  }
  ...
  async getBidCountsForListings(listingIds: number[]): Promise<Record<number, number>> {
    ...
    result.forEach(row => counts[row.listingId] = row.count);
  ...
  // Notifications operations
  async getNotificationsByUser(userId: number): Promise<Notification[]> {
    // Упрощенная версия без сложного join для предотвращения ошибок
    const result = await db
      .select()
      .from(notifications)
  ...
  async createCarAlert(insertCarAlert: InsertCarAlert): Promise<CarAlert> {
    const [alert] = await db.insert(carAlerts).values(insertCarAlert).returning();
    return alert;
  }
  ...
  async getAllCarAlerts(): Promise<CarAlert[]> {
    return await db.select().from(carAlerts);
  }

  // Banners operations
  async getBanners(): Promise<Banner[]> {
    return await db.select().from(banners).where(eq(banners.isActive, true)).orderBy(banners.order);
  }
  ...
  // SellCarSection operations
  async getSellCarSection(): Promise<SellCarSection | undefined> {
    const result = await db.select().from(sellCarSection).limit(1);
    return result[0];
  }

  async updateSellCarSection(data: Partial<InsertSellCarSection>): Promise<SellCarSection | undefined> {
    const existing = await this.getSellCarSection();
    if (existing) {
      const [updated] = await db
        .update(sellCarSection)
        .set({ ...data, updatedAt: new Date() })
        .where(eq(sellCarSection.id, existing.id))
        .returning();
      return updated;
    } else {
      const [created] = await db.insert(sellCarSection).values(data as InsertSellCarSection).returning();
      return created;
    }
  }

  // AdvertisementCarousel operations
  async getAdvertisementCarousel(): Promise<AdvertisementCarousel[]> {
  ...
  async createAdvertisement(insertAd: InsertAdvertisementCarousel): Promise<AdvertisementCarousel> {
    const [ad] = await db.insert(advertisementCarousel).values(insertAd).returning();
    return ad;
  }

  async updateAdvertisement(id: number, data: Partial<InsertAdvertisementCarousel>): Promise<AdvertisementCarousel | undefined> {
    const [ad] = await db
      .update(advertisementCarousel)
      .set({ ...data, updatedAt: new Date() })
      .where(eq(advertisementCarousel.id, id))
  ...
  async getConversationExists(buyerId: number, sellerId: number, listingId: number): Promise<Conversation | undefined> {
  ...
  async getRecentWonListings(hoursLimit: number = 5): Promise<CarListing[]> {
    const hoursAgo = new Date(Date.now() - hoursLimit * 60 * 60 * 1000);
    return await db.select().from(carListings)
      .where(
        and(
          eq(carListings.status, 'ended'),
          gte(carListings.endedAt, hoursAgo)
        )
      )
  ...
  async getAllListings(): Promise<CarListing[]> {
    return await db.select().from(carListings).orderBy(desc(carListings.createdAt));
  }

  async getAllActiveListings(): Promise<CarListing[]> {
    return await db.select().from(carListings)
      .where(eq(carListings.status, 'active'))
...
// Используем PostgreSQL везде для предотвращения потери данных
console.log('🔄 Переключение на единую PostgreSQL базу данных...');
