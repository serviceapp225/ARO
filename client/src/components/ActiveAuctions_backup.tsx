...
export function ActiveAuctions({ searchQuery = "", customListings }: ActiveAuctionsProps) {
  ...
  useEffect(() => {
    if (loading) {
      ...
      const timeout = setTimeout(() => {
        console.warn('⚠️ Принудительное обновление - слишком долгая загрузка');
        setLoadingTimeout(true);
        refreshAuctions();
      }, 10000); // 10 секунд максимум загрузки
    ...
    <div className="mb-20">
      ...
      {displayedAuctions.length === 0 && !loading ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-gray-600 mb-2">Нет активных аукционов</h3>
          <p className="text-gray-500 mb-4">Новые автомобили появятся скоро</p>
          <Button 
            onClick={() => {
