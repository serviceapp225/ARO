...
export default function MySalesSimple() {
  const { user } = useAuth();
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                            ...
                            {listing.status === 'active' ? 'Активно' : 
                             listing.status === 'pending' ? 'На модерации' : 'Завершено'}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-4 text-sm text-gray-600 mb-3">
                          <span className="font-medium">
                            {listing.currentBid 
                              ? `${parseFloat(listing.currentBid).toLocaleString()} сомони`
                              : `${parseFloat(listing.startingPrice).toLocaleString()} сомони`}
                          </span>
                        </div>
