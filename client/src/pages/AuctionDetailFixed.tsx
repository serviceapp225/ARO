...
export default function AuctionDetail() {
  ...
  const bidMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      // Show success toast - автоисчезновение через 1 секунду, нельзя закрыть
      toast({
  ...
  useEffect(() => {
    if (lastBidUpdate && lastBidUpdate.listingId === parseInt(id || '0')) {
      ...
      if (lastBidUpdate.data?.bid?.amount) {
        ...
        // Показываем уведомление о новой ставке - автоисчезновение через 1 секунду, нельзя закрыть
        toast({
          title: "🔥 Новая ставка!",
  ...
  const handleConfirmBidOLD = async () => {
    ...
    if (shouldShowEffects) {
      ...
      try {
        // Генерируем звук успешной ставки
        try {
          const audioContext = new (window.AudioContext || (window as any).webkitAudioContext)();
          const duration = 1.5;
          const sampleRate = audioContext.sampleRate;
            ...
            <CardContent className="p-6">
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div className="flex justify-between items-center py-2 px-3 rounded-lg bg-green-50 border border-green-200 transition-all duration-300">
