...
export function useOptimizedRealTime(config: RealTimeConfig = {}) {
  ...
  const smartUpdateAuctionData = useCallback(async (listingId?: string) => {
    ...
    try {
      if (listingId) {
        ...
        // ВРЕМЕННО ОТКЛЮЧЕНО для решения проблем с деплоем - слишком частые запросы
        console.log('🔄 Фоновое обновление временно отключено для стабилизации деплоя');
        
        // Используем только кэш invalidation без прямых запросов
  ...
  const handleWebSocketMessage = useCallback((message: any) => {
    if (message.type === 'bid_update') {
      ...
      // ВРЕМЕННО ОТКЛЮЧЕНО: Принудительное обновление через smartUpdateAuctionData для стабилизации деплоя
      console.log('🔄 Принудительное обновление временно отключено');
    }
  }, [smartUpdateAuctionData]);
